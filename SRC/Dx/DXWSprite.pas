...
type
  ...
  public
    Name:string;
    //pid: integer;
    //oid: integer; // index dans la liste mObjs to update pid
    ...
    //property L: Byte read FL write FL;
  ...
  public
    ...
    property Deading: Integer read FDeading write FDeading;
  end;
...
function TSprite.GetSpriteAt(X, Y: Integer): TSprite;
  ...
  begin
    ...
    begin
      if (Result=nil) then // or (Sprite.Z>Result.Z) then
        Result := Sprite;
    end;
...
const
  ...
  RD: array [-1..7] of byte =  (90,255,0  ,140,34  ,255,120,45  ,235  );
  GR: array [-1..7] of byte =  (90,40 ,0  ,90,140  ,128,60 ,75  ,100  );
  BL: array [-1..7] of byte =  (90,40 ,210,43,34  ,64,120,110  ,  100 );
begin
  ...
  begin
    ...
    NewCol.rgbReserved := 1;
    with Image.Picture.Bitmap do
      SetDIBColorTable(Canvas.Handle, 1, 1, NewCol);   // put in colortable a new color=plcolor
    Image.Restore;
  end;
  if mobjs[oid].Deading=-1
  then Image.Draw(FEngine.Surface, r.Left, r.Top, ImageIndex)
  else
  begin
    mobjs[oid].Deading:=mobjs[oid].Deading-10;
    Image.DrawAlpha(FEngine.Surface, r, ImageIndex,mobjs[oid].Deading);
    if mobjs[oid].Deading < 10 then Dead;
  end;
end;
{----------------------------------------------------------------------------}
...
{----------------------------------------------------------------------------}
procedure TCustomDXSpriteEngine.Scroll(dx,dy:integer);
var
  NewX, NewY: double;
